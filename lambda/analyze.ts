...
export const handler = async (event: LambdaEvent): Promise<LambdaResponse> => {
  ...
  try {
    const result = await analyzeWithBedrock(req);
    return {
      statusCode: 200,
      headers: CORS_HEADERS,
...
async function analyzeWithBedrock(req: AnalyzeRequest): Promise<AnalyzeResult> {
...
function parseBedrockResponse(text: string, usedRealAI: boolean): AnalyzeResult {
  ...
  return { wedgeAnalysis: text, minorityRisk: "", regulations: "", usedRealAI };
}
